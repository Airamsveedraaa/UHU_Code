...
void generarVector(vector<int>& v, int tam, const string& distribucion, unsigned int semilla) {
    ...
    sort(v.begin(), v.end()); // Ordenamos siempre
}
...
void realizarPruebas(const vector<int>& v, const string& distribucion, ofstream& archivo) {
    ...
    // Valores representativos para buscar
    int valores_buscar[] = {
        copia.front(),      // Primer elemento
        copia.back(),       // Último elemento
        copia[n / 2],       // Elemento medio
        copia.back() + 100  // Valor que no existe
    };
    ...
    for (int x : valores_buscar) {
        ...
        archivo << n << ";" << distribucion << ";"
            << OE_BIN << ";"<< OE_INTER << ";" 
           << fixed << setprecision(8) << tiempo_bin << ";" << tiempo_inter << "\n";
...
int main() {
    ...
    // Encabezados del CSV
    archivo << "Tamano;Distribucion;OE_Binaria;OE_Interpolacion;Tiempo_Binaria(s);Tiempo_Interpolacion(s)\n";
