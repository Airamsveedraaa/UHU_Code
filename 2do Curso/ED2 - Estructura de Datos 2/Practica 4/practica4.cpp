...
{
    typename Grafo<T,float>::ConjVertices copiaV=G.vertices();
    T verticeMayor=copiaV.quitar().getObj();
    float maximoActual=0;
    while(!copiaV.esVacio())
    {
        Vertice<T> v=copiaV.quitar();
        float suma=0;
        ...
        if(suma > maximoActual)
        {
            maximoActual=suma;
            verticeMayor=v.getObj();
        }
    }
    return verticeMayor;
}
...
{
    typename Grafo<T,U>::ConjVertices copia=G.vertices();
    while(!copia.esVacio())
    {
        Vertice<T> v=copia.quitar();
        if(G.antecesores(v.getObj()).esVacio())
        {
            cout << v.getObj() << endl;
        }
    }
