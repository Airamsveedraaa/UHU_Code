...
public class BusquedaAEstrella extends Busqueda {
	...
	public int calcularH(int f, int c, Entorno mapa) {
		...
		if (!propiaH) {
			...
			// si despF es positivo nos indica que debemos ir hacia arriba para encontrar la
			// salida, se penaliza ese movimiento de esa manera con despF*(-2)
			// haciendo que ese movimiento se elija menos, si es menor a 0 se deja despF tal
			// cual, indica que debemos ir hacia abajo que no está penalizado.
	...
	private Queue<Nodo> getSucesores(Nodo Actual, Entorno mapa) {
		...
		if (mapa.esTransitable(f - 1, c)) {
			int h = calcularH(f - 1, c, mapa); // heurística de Distancia Manhattan o distancia Manhattan Ponderada,
	...
	public List<String> resolver(Entorno mapa) {
		...
		int Hinicial = calcularH(mapa.agenteF, mapa.agenteC, mapa); // el nodo inicial también tiene h que hay que
																	// calcular
		...
		System.out.println(" Origen: " + "(" + inicial.f + "," + inicial.c + ")  -> Meta: " + "(" + mapa.metaF + ","
				+ mapa.metaC + ")\n\n");
		while (Abiertos.size() > 0) {
			int tam = Abiertos.size();
			if (tam > this.tamMax)
				this.tamMax = tam; // obtenemos de esta forma el máximo número de nodos que ha tenido abiertos en
									// toda la ejecución
			super.nAbiertos++; // para mostrar el numero de nodos expandidos en toda la ejecución.
			Nodo Actual = Abiertos.remove();
			Cerrados.add(Actual);
			System.out.println("[PASO " + paso + " ]");
			System.out.println("Seleccionado: " + Actual + " [f= " + Actual.valorF + ", g= " + Actual.g + ", h= "
					+ Actual.h + " ]");

			if (mapa.esMeta(Actual.f, Actual.c)) {
				// si hemos llegado a la meta, calculamos el tiempo de ejecución y reconstruimos
				// el camino para mostrarlo.
				...
				// si aun no es meta, obtenemos los sucesores, tratamos los repetidos y volvemos
			...
			// importante, en cada pasada se muestra los sucesores y el estado de las
			// estructuras
			System.out.println("Estado ABIERTOS: " + Abiertos);
			System.out.println("Estado CERRADOS: " + Cerrados);
			paso++;
		}
		// si no encuentra la solucion se calcula el tiempo de ejecucion y se muestra el
