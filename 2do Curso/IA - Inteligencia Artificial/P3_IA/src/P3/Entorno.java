...
class Entorno {
    ...
    public boolean esTransitable(int f, int c) {
        if (f < 0 || f >= filas || c < 0 || c >= cols)
            return false;
        char celda = grid[f][c];
        // Los jugadores no pueden pisar paredes '#', rastros 'a'/'b', genÃ©ricos '.', ni
        // chocar
    ...
    public void deshacerMovimiento(char jugador, String accion) {
        ...
        // 1. La celda actual queda vacÃ­a de nuevo
        grid[f][c] = ' ';

        // 2. Calculamos de dÃ³nde vino retrocediendo la acciÃ³n
        int origF = f;
        int origC = c;
        if (accion.equalsIgnoreCase("N"))
            origF++;
        ...
        // 4. Restauramos el jugador a su celda orÃ­gen (pisando su antiguo rastro)
